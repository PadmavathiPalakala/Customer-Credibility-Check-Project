...
# -*- coding: utf-8 -*-
"""
Created on Fri March 29 12:50:04 2021

@author: Padmavathi Palakala
"""
...
def main():
    st.title("Customer Credibility Prediction")
    html_temp = """
    <div style="background-color:#FF0000;padding:10px">
    <h2 style="color:Black;text-align:center;">Customer Credibility Predictor App By Padmavathi Palakala</h2>
    </div>
    """
    ...
    ApplicantIncome = st.number_input(label='Enter your Monthly Salary in GBP', min_value=None, max_value=None)
    ai=ApplicantIncome
    CoapplicantIncome = st.number_input(label='Enter Co-applicant Monthly Salary in GBP', min_value=None, max_value=None,format='%f')
    ci=CoapplicantIncome
    LoanAmount = st.number_input(label='Enter LoanAmount Required in GBP', min_value=None, max_value=None,format='%f')
    la=LoanAmount
    Loan_Amount_Term = st.number_input(label='Enter LoanTerm in Months', min_value=12, max_value=None,format='%f')
    ...
    st.success(result)
    st.subheader("Awesome Webapps by Padmavathi Palakala")
